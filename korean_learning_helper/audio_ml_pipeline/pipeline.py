...
def step_filter_reindex(rich_path: str, words_path: str) -> int:
    ...
    os.rename(temp_csv_path, words_path)
...
def step_generate_audio(words_path: str, audio_csv_path: str, audio_dir: str) -> None:
    ...
    Note: Machine learning models (torch, qwen_tts) and soundfile are loaded
    lazily inside this function only if audio generation is required. This
    prevents unnecessary memory overhead when all audio files already exist.
    """
    ...
    temp_csv_path = audio_csv_path + ".tmp"
    with open(temp_csv_path, "w", encoding="utf-8", newline="") as f_out:
    ...
    os.rename(temp_csv_path, audio_csv_path)
...
def main() -> None:
    ...
    parser.add_argument(
        "--rich", default="topik_words_rich.csv",
        help="Source vocabulary CSV with user-selected index in col 0"
    )
    parser.add_argument(
        "--words", default="my_topik_words.csv",
        help="Filtered & reindexed output"
    )
    parser.add_argument(
        "--audio-csv", default="my_topik_words_with_audio.csv",
    ...
    parser.add_argument(
        "--anki", default="my_topik_words_anki.tsv",
        help="Final Anki TSV output"
    )
    parser.add_argument(
        "--anki-csv", default="my_topik_words_anki.csv",
